    ...
    try:
        ...
        if command == Command.create_new_db:
            ...
            Application.run_study(
                db_name=kwargs[opt.database],
                out_dir=kwargs[opt.out_dir],
                study_list=kwargs[opt.study],
...
@run.command()
@click.option(f"--{opt.database}", help="Name of the database to be created")
@click.option(
    f"--{opt.db_type}",
    default="SQL",
...
@run.command()
@click.option(f"--{opt.database}", help="Name of the database to append data to")
@click.option(
    f"--{opt.url}",
    default=None,
    help="URL obtained from the TriNetX export option. "
...
@run.command()
@click.option(f"--{opt.database}", help="Name of the database to select data from")
@click.option(
    f"--{opt.study}",
    multiple=True,
...
@click.option(
    f"--{opt.out_dir}", help="Path to the directory where the results will be stored."
)
@click.option(
    f"--{opt.local_access}",
    default=True,
