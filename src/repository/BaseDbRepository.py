...
class BaseDbRepository(metaclass=ABCMeta):
    ...
    def _get_codes_info(
        ...
        patient_groups = (
            self._group_patient_params(date_patient_map) if date_patient_map else [None]
        )

        params = [
            (
                event.codes,
                event.get_data_table(),
                columns,
                patient_info,
                include_icd9,
                first_incident,
                event.negation,
                event.include_subcodes,
                event.num_value,
                event.text_value,
            )
    ...
    def __get_code_info_job(
        ...
        if not codes:
            return self.__get_all_codes_info(
                table_name,
                columns,
                patients_info,
                first_incident,
        ...
        if negation_event:
            df = self.__process_negative_codes(
                positive_codes_df=df, patients_info=patients_info, codes=codes
            )
    ...
    def __get_icd9_mapped_code_info(
        self,
        icd10_codes: list,
        table_name: str,
        columns: list,
        ...
        icd10_subcodes = (
            self.db_manager.request_subcodes(icd10_codes, table_name)
            if include_subcodes
            else icd10_codes
        )
        if icd10_subcodes:
        ...
        icd10_to_icd9_map_df = self.db_manager.request_icd9_icd10_map(
            icd10_codes, search_column=cc.icd10_code
        )
        ...
        if cc.code in icd9_df.columns:
            ...
            icd9_df = icd9_df.merge(
                icd10_to_icd9_map_df, "inner", left_on=cc.code, right_on=cc.icd9_code
            )
            icd9_df[cc.code] = icd9_df[cc.icd10_code]
            icd9_df = icd9_df.drop(
                columns=[cc.icd9_code, cc.icd10_code]
            ).drop_duplicates()

        return icd9_df

    def _group_patient_params(
        self, date_patient_map: list, cohort_size: int = 10_000
    ) -> list:
        """
        ...
        """
        self.logger.debug("group patients for request")
        res = []
        counter = 0
        curr_patients = []
        for record in date_patient_map:
            min_date = (
                ...
                else record["start_date"].strftime("%Y-%m-%d")
            )
            max_date = (
                None
                if record["end_date"] is None
                else record["end_date"].strftime("%Y-%m-%d")
            )
            ...
            if patients_number > 1.5 * cohort_size:
                ...
                res.append(
                    [
                        (min_date, max_date, record["patients"][i : i + cohort_size])
    ...
    def __process_negative_codes(
        ...
        self.logger.debug("process_negative_codes")

        positive_codes_df[cc.date] = pd.to_datetime(
            positive_codes_df[cc.date].astype(str)
        )
        ...
        patient_date_range_df = patient_date_range_df.merge(
            positive_codes_df, how="left", on=cc.patient_id
        )
        ...
        patient_date_range_df = (
            patient_date_range_df[patient_date_range_df[col_within_time] == 0]
            .drop_duplicates(subset=[cc.patient_id, col_max_date])
            .drop(columns=[cc.date, col_within_time])
        )
        patient_date_range_df = patient_date_range_df.rename(
            columns={col_max_date: cc.date}
        )
        ...
        clear_cols = [
            x
            for x in patient_date_range_df.columns
    ...
    def __process_positive_event_codes(
        self,
        codes: Optional[list],
        table_name: str,
        ...
        if (
            ...
            if first_incident:
                df = df[
                    df.index
                    == df.groupby([cc.patient_id, cc.code])[cc.date].transform("idxmin")
                ]
