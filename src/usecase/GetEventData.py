...
class GetEventData:
    ...
    def execute(
        ...
        columns: list,
        date_patient_map: Optional[list],
        etf: Optional[ExperimentTimeFrame],
        ...
        def adjust_columns(event, cols):
            if event.category == EventCategory.Medication:
                ...
                event.category == EventCategory.LabResult
                or event.category == EventCategory.VitalSign
            ):
                cols = cols + [cc.num_value, cc.text_value]
            return cols
        ...
        res_data = ConcurrentUtil.do_async_job(
            self.__request_event_info, params_list=params
        )
    ...
    def __adjust_event_period(
        ...
        for record in date_patient_map:
            if record["date"] is None:
                ...
                days = (
                    event.period.min_t if event.period.min_t is not None else -365_000
                )
            ...
            if etf is not None:
                ...
                if etf.min_date is not None:
                    ...
                    start_date = (
                        max(start_date, min_d) if start_date is not None else min_d
                    )
    ...
    def __request_event_info(
        ...
        if event.category == EventCategory.Patient:
            ...
            if EventConstant.DEATH in event.codes:
                df = self.__patient_repo.get_dead_patients(
                    columns=patient_columns, date_patient_map=date_patient_map
                )
                if df is None:
                    return None
            ...
            df = self.__event_repo.get_event_info(
                event=event,
                columns=columns,
                date_patient_map=date_patient_map,
        ...
        self.logger.debug(
            f"Returning event info for event: {event.id}, data size: {0 if df is None else df.shape}"
        )
        return df

    def __get_attribute_events(
        self, df: pd.DataFrame, event: Event, exclude: bool = False
    ) -> Optional[pd.DataFrame]:
        # if no event to filter return df as is
        if (exclude and not event.exclusion_events) or (
            not exclude and not event.having_events
        ):
            return None
    ...
    def __build_date_patient_map_for_event(
        self, event: Event, df: pd.DataFrame, common_map: list
    ) -> list:
        if event.period is None:
            return common_map
        gdf = df.groupby(cc.date)[cc.patient_id].apply(list).reset_index()
    ...
    def __filter_excluded_events(
        self, df: pd.DataFrame, excl_df: pd.DataFrame, event: Event
    ) -> Optional[pd.DataFrame]:
        ...
        for excl_event in event.exclusion_events:
            ...
            if event.exclusion_period is None and excl_event.period is None:
                ...
                min_t = (
                    excl_event.period.min_t
                    if excl_event.period is not None
                    else event.exclusion_period.min_t
                )
                max_t = (
                    excl_event.period.max_t
                    if excl_event.period is not None
                    else event.exclusion_period.max_t
                )
        ...
        if len(filtered_dfs) > 1:
            ...
            if event.exclusion_mode == AttributeMode.any:
                ...
                for curr_df in filtered_dfs[1:]:
                    excl_df = excl_df.merge(
                        curr_df, on=[cc.patient_id, cc.date], how="inner"
                    )
            else:
        ...
        df = df.drop(
            columns=[cc.date + "_excl", cc.event_id + "_excl", "date_diff"]
        ).drop_duplicates()
        self.logger.debug(f"Excluded {rows_count - df.shape[0]} records")
        return df

    def __filter_having_events(
        self, df: pd.DataFrame, having_df: pd.DataFrame, event: Event
    ) -> Optional[pd.DataFrame]:
        rows_count = df.shape[0]
        ...
        for having_event in event.having_events:
            ...
            if event.having_period is None and having_event.period is None:
                ...
                min_t = (
                    having_event.period.min_t
                    if having_event.period is not None
                    else event.having_period.min_t
                )
                max_t = (
                    having_event.period.max_t
                    if having_event.period is not None
                    else event.having_period.max_t
                )
    ...
    def __filter_attribute_evens(
        self, df: pd.DataFrame, event: Event
    ) -> Optional[pd.DataFrame]:
        self.logger.debug(f"Filter attribute events for {event.id}")

        if df is not None and event.exclusion_events:
            self.logger.debug(
                f"exclude patients with {[e.id for e in event.exclusion_events]} "
        ...
        if df is not None and event.having_events:
            self.logger.debug(
                f"Filter patients with {event.having_events} "
