...
from src.datamodel.DataColumns import (
    DataDictionaryColumns as dd_cols,
    CommonColumns as cc_cols,
...
class ConvertDataModel:
    ...
    def convert_tnx_to_common_async(self, src_path: str) -> str:
        self.logger.debug("Async Converting TNX data to common format")
        ...
        self.logger.debug("Converting TNX files to common format")
        ...
        params = [
            (
                ...
                src_path,
                dest_path,
                conversion_map[conversion_map[map_cols.tnx_file] == f],
            )
            for f in tnx_files
        ]
    ...
    def convert_tnx_to_common(
        self, file_name: str, src_path: str, dest_path: str, table_map: pd.DataFrame
    ):
        self.logger.debug(
            f"Converting file {file_name} to from {src_path} to {dest_path}"
        )
        if not os.path.exists(src_path) or not os.path.exists(
        ...
        columns_map = {
            tnx_c: cm_c
            for tnx_c, cm_c in zip(
                table_map[map_cols.tnx_column], table_map[map_cols.column]
            )
        }
        ...
        if file_name == "patient.csv":
            ...
            df[cc_cols.date_of_birth] = pd.to_datetime(
                df[cc_cols.date_of_birth], format="%Y"
            )
            df[cc_cols.date_of_death] = (
                pd.to_datetime(df[cc_cols.date_of_death], format="%Y%m")
                + pd.offsets.MonthEnd()
            )
        else:
    ...
    def extract_files_from_zip(
        self, filename: str, archive_name: str, extraction_path: str
    ):
        self.logger.debug(
            f"Extract {filename} from {archive_name} to {extraction_path}"
        )
        with zipfile.ZipFile(archive_name, "r") as zip_ref:
            if filename not in zip_ref.namelist():
                print(
                    f"Process {os.getpid()}: archive {archive_name} does not contain {filename}"
                )
                return
