    ...
    def validate(self, config_file_name: str) -> bool:
        ...
        if config.time_frame is not None:
            try:
                ...
                self.logger.error(
                    f"Invalid time frame format in config file: {config_file_name}. "
        ...
        for l in config.levels:
            if l.level < 0:
                self.logger.error(
                    f"Invalid level number: {l.level} in config file: {config_file_name}"
                )
                return False
            if not l.events:
                self.logger.error(
                    f"Level {l.level if l.name is None else l.name} in config file: {config_file_name} "
                    f"should have at least one event"
                )
                return False
            if any([e.id is None or e.category is None for e in l.events]):
                self.logger.error(
                    f"Invalid event in level {l.level} in config file: {config_file_name}. "
