...
def execute(
    ...
    if len(pt) == 1:
        tables.remove(pt[0])
        upload_table_process(
            app_config, local_access, database, pt[0], archive_name, set_index
        )

    # upload all other tables in separate processes
    params = [
        (app_config, local_access, database, t, archive_name, set_index) for t in tables
    ]
    ConcurrentUtil.run_in_separate_processes(
        upload_table_process, params, max_processes=len(params)
    )


def upload_table_process(
    app_config: AppConfig,
    local_access: bool,
    database: str,
    table: SqlTable,
    ...
    print(
        f"Process {os.getpid()} table {table.name}: Run thread for table {table.name}"
    )
    if table.src_file[-4:] != ".csv":
        return
    print(
        f"Process {os.getpid()} table {table.name}: Read data file {table.src_file} from {archive_name}"
    )

    db_manager = DatabaseManager(
        app_config, local_access=local_access, db_name=database
    )
...
def import_code_mapping_data(db_manager: DatabaseManager, table: SqlTable):
    print("import code mapping data")
    if table.src_file[-4:] != ".csv":
        return
