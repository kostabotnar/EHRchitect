...
from src.datamodel.ExperimentConfig import (
    ExperimentConfig,
    ExperimentLevel,
    ExperimentTimeFrame,
...
class FindEventsChain:
    ...
    def __build_events_chain(
        ...
        date_patient_map = self.__init_date_patient_map(
            experiment_config.time_frame, patients
        )
        ...
        for i, experiment_level in enumerate(experiment_config.levels):
            ...
            df = self.__get_events_data(
                experiment_level,
                date_patient_map,
                experiment_config.time_frame,
                include_icd9,
                first_incident,
            )
            ...
            if i < n_levels - 1:
                ...
                date_patient_map = self.__build_date_patient_map(
                    df, next_level_dist, experiment_config.time_frame
                )
            # rename columns with level number
            curr_level_columns = [
                cc.get_column_at_level(c, curr_level_number)
                for c in df.columns
                if c != cc.patient_id
            ]
            ...
            if curr_level_number == 0:
                ...
                chain_df = self.__merge_levels(
                    index_df=chain_df,
                    target_df=df,
                    index_level=experiment_config.get_level_by_number(
                        curr_level_number - 1
                    ),
                ...
                file_dir, file_name = (
                    self.__file_provider.get_transition_group_location(
            ...
            curr_level_columns = [
                c for c in chain_df.columns if c.endswith(f"_{curr_level_number}")
            ]
            ...
            self.__save_to_file(
                df=chain_df, file_dir=file_dir, file_name=file_name, index=cc.patient_id
            )

    def __get_events_data(
        self,
        level: ExperimentLevel,
        date_patient_map: Optional[list] = None,
        ...
        df = GetEventData(self.__patient_repo, self.__event_repo).execute(
            level, columns, date_patient_map, etf, include_icd9, first_incident
        )
        if df is None:
    ...
    def __match_events(
        ...
        index_level: ExperimentLevel,
        target_level: ExperimentLevel,
    ) -> Optional[pd.DataFrame]:
        self.logger.debug(
            f"__match_events: between levels {index_level.level} and {target_level.level}"
        )

        if target_df is None or target_df.empty or index_df is None or index_df.empty:
            # nothing to match
            self.logger.warning(
                f" Nothing to match between level {index_level.level} and {target_level.level}"
            )
            return None
        ...
        for i, curr_patients in enumerate(patients):
            self.logger.debug(
                f"match {i * bin_size}..{(i + 1) * bin_size} from {total_patients}"
            )
            ...
            if target_level.match_mode == MatchMode.first_match:
                ...
                curr_index_df = curr_index_df[
                    curr_index_df.index
                    == curr_index_df.groupby(group_col)[col_index_date].transform(
                        "idxmin"
                    )
                ]
    ...
    def __filter_events_within_period(
        self, df: pd.DataFrame, start_level: ExperimentLevel, end_level: ExperimentLevel
    ):
        self.logger.debug(
        ...
        for event in end_level.events:
            ...
            if period is None:
                ...
                event_time_mask = (
                    time_mask
                    & (df_event[col_distance] >= min_t)
                    & (df_event[col_distance] <= max_t)
                )
                # for negative events get only records with maximal distance
                event_time_mask = event_time_mask | (
                    is_negative_code & (df_event[col_distance] == max_t)
                )
            else:
                event_time_mask = (
                    time_mask
                    & (df_event[col_distance] >= 0)
                    & (df_event[col_distance] <= period)
                )
                # for negative events get only records with maximal distance
                event_time_mask = event_time_mask | (
                    is_negative_code & (df_event[col_distance] == period)
                )
    ...
    def __save_to_file(
        ...
        self.__file_provider.save_dataframe_file(
            df=df, file_dir=file_dir, filename=file_name, file_format=file_format
        )
    ...
    def __init_date_patient_map(self, etf: ExperimentTimeFrame, patients: list) -> list:
        ...
        if etf is None:
            ...
            min_d = (
                None
                if etf.min_date is None
                else datetime.datetime.strptime(str(etf.min_date), "%Y-%m-%d")
            )
            max_d = (
                None
                if etf.max_date is None
                else datetime.datetime.strptime(str(etf.max_date), "%Y-%m-%d")
            )
        return [
            {"start_date": min_d, "end_date": max_d, "date": None, "patients": patients}
        ]

    def __add_time_interval(
        self,
        event_dates: pd.Series,
        time_interval: Union[int, ExperimentTimeInterval, type(None)],
    ) -> tuple:
    ...
    def __build_date_patient_map(
        self,
        df: pd.DataFrame,
        time_interval: Union[int, ExperimentTimeInterval, type(None)],
        etf: ExperimentTimeFrame,
    ) -> list:
        self.logger.debug(
            f"__build_date_patient_map: t={time_interval} time frame = {etf}"
        )
        ...
        if time_interval is not None:
            start_dates, end_dates = self.__add_time_interval(
                df[cc.date], time_interval
            )
    ...
    def __merge_levels(
        ...
        if target_df is None or target_df.empty:
            return None
        self.logger.debug(
            f"merge level {index_level.level} with {index_df.shape} records "
        ...
        if index_df is None or index_df.empty:
            self.logger.warning(
                f"Nothing was matched between {index_level.level} and {target_level.level} levels"
            )
            return None
