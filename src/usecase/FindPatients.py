...
class FindPatients:
    ...
    def execute(
        self, study_config: Optional[ExperimentConfig] = None, include_icd9: bool = True
    ) -> Optional[list]:
        """
        Find all patients for index level and split them on groups
        """
        self.logger.debug(
            f"execute: experiment_config = {study_config} include_icd9 = {include_icd9}"
        )
        ...
        patients_df = patients_df.sort_values(
            by=[cc.patient_id], ascending=[True]
        ).set_index(cc.patient_id)

        file_dir, file_name = self.__file_provider.patients_metadata_file_location(
            study_config.outcome_dir
        )
        self.__file_provider.save_dataframe_file(
            df=patients_df, file_dir=file_dir, filename=file_name
        )
    ...
    def __get_index_patients(
        self, index_level: ExperimentLevel, include_icd9: bool = True
    ) -> Optional[list]:
        self.logger.debug(
            f"get index event patients for {len(index_level.events)} events"
        )
        columns = [cc.patient_id, cc.code, cc.date]
        first_incident = index_level.match_mode == MatchMode.first_match
        res_data = GetEventData(self.__patient_repo, self.__event_repo).execute(
            index_level, columns, None, None, include_icd9, first_incident
        )
