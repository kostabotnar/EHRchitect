...
def create_db(
    ...
    if new_db:
        ...
        code_map_table = table_creator.create_code_map_table(
            db_name, fp.code_map_table_file
        )

    # upload data to the DB
    if code_map_table is not None:
        ImportDataToDB.import_code_mapping_data(db_manager, code_map_table)
    ImportDataToDB.execute(
        app_config, local_access, db_name, tables_data, archive, set_index
    )
...
def run_study(db_name: str, out_dir: str, study_list: list, local_db: bool = True):
    ...
    if db_name not in dbs:
        logger.error(
            f"Database {db_name} not found in the config.\nAvailable databases: {dbs}"
        )
        return
    ...
    for config_file_name in study_list:
        ...
        patient_groups = FindPatients(patient_repo, event_repo).execute(
            study_config, include_icd9
        )
        ...
        params = [
            (app_config, pg, study_config, db_name, include_icd9, local_db)
            for pg in enumerate(patient_groups)
        ]
...
def download_dataset(url: str, archive: str):
    ...
    if url is None or archive is None:
        logger.error("Download error: url or archive is not defined.")
        return False
...
def create_study_outcome_file_structure(study_config: ExperimentConfig):
    ...
    for i in range(len(study_config.levels)):
        e_path = fp.get_events_path(
            outcome_dir=study_config.outcome_dir, level_number=i
        )
        e_path.mkdir(parents=True, exist_ok=True)
        if i == 0:
            continue
        t_path = fp.get_transitions_path(
            outcome_dir=study_config.outcome_dir, end_level_number=i
        )
        t_path.mkdir(parents=True, exist_ok=True)


def find_event_chain_async(
