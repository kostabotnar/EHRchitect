...
def upload_table_from_file(db_name: str, table_name: str, file_name: str) -> str:
    return (
        f"LOAD DATA LOCAL INFILE '{file_name}' INTO TABLE {db_name}.{table_name} "
...
def request_dead_patients(
    patients_info: Optional[list] = None, columns: Optional[list] = None
) -> str:
    columns = SqlUtil.selected_columns_expr(columns)
    # date-patient condition
    date_patient_condition = compose_date_patient_condition(
        patients_info, cc.date_of_death, ct.patient
    )

    # add date condition
    where = f"where not isnull({cc.date_of_death})"
    where += f" and ({date_patient_condition})" if date_patient_condition else ""
    return f"select {columns} from {ct.patient} {where}"
...
def compose_date_patient_condition(
    patients_info: list, date_column: str, table: str
) -> Optional[str]:
    ...
    for min_date, max_date, patients in patients_info:
        condition = (
            [SqlUtil.in_expression(patient_id_col, patients)] if patients else []
        )
...
def get_code_info(
    ...
    if cc.type in columns:
        from_expr = (
            f"{table} left join {ct.encounter} "
    ...
    if codes:
        ...
        codes_condition = (
            SqlUtil.like_expression(request_code_column, codes)
            if include_subcodes
            else SqlUtil.in_expression(request_code_column, codes)
        )
