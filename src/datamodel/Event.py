...
from src.datamodel.DataColumns import CommonTables as ct
...
class EventTimeInterval:
    ...
    @staticmethod
    def from_experiment_event_period(
        exp_p: Optional[ExperimentTimeInterval],
    ) -> Optional[EventTimeInterval]:
        if exp_p is None:
            return None
...
class AttributeMode(Enum):
    ...
    @classmethod
    def from_attr_event_mode(cls, mode: AttributeEventMode) -> AttributeMode:
        if mode == AttributeEventMode.any:
            return cls.any
        if mode == AttributeEventMode.all:
            return cls.all
...
class Event:
    ...
    inclusion_mode: AttributeMode = AttributeMode.any
    exclusion_mode: AttributeMode = AttributeMode.any
    ...
    def __hash__(self):
        return hash(
            (
                ...
                tuple(self.codes),
                self.negation,
                self.include_subcodes,
                self.exclusion_period,
                tuple(self.exclusion_events),
                tuple(self.having_events),
                self.having_period,
                self.period,
            )
        )
    ...
    def from_experiment_event(experiment_event: ExperimentEvent) -> Optional[Event]:
        ...
        if experiment_event.exclude and experiment_event.exclude.events:
            excl_period = EventTimeInterval.from_experiment_event_period(
                experiment_event.exclude.period
            )
            excl_events = [
                Event.from_experiment_event(e) for e in experiment_event.exclude.events
            ]
            excl_mode = AttributeMode.from_attr_event_mode(
                experiment_event.exclude.mode
            )
        else:
            excl_period = None
            excl_events = None
            excl_mode = AttributeMode.any

        if experiment_event.having and experiment_event.having.events:
            having_period = EventTimeInterval.from_experiment_event_period(
                experiment_event.having.period
            )
            having_events = [
                Event.from_experiment_event(e) for e in experiment_event.having.events
            ]
            incl_mode = AttributeMode.from_attr_event_mode(experiment_event.having.mode)
        else:
            having_period = None
            having_events = None
            incl_mode = AttributeMode.any
